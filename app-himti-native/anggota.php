...
if ($cmdHapus == "Hapus" && $hdnID != "") {
    ...
    if ($koneksi_db->query($sql_delete_anggota)) {
        $notifikasi = "<div class='alert alert-success

    alert-dismissible fade show' role='alert'>Sukses menghapus sebuah
    data anggota.<button type='button' class='btn-close'
    data-bs-dismiss='alert' aria-label='Close'></button></div>";
    } else {
        $notifikasi = "<div class='alert alert-danger

    alert-dismissible fade show' role='alert'>Terjadi kesalahan pada
    sistem. Data tidak dapat dihapus.<button type='button'
...
if ($q == "") {
    ...
    $sql_select_anggota = "SELECT * FROM anggota WHERE
kode_anggota='" . $q . "' OR nama_anggota LIKE '%" . $q . "%'";
}
...
table-hover">
                    ...
                    while ($data_select_anggota = $kueri_select_anggota->fetch_array()) {
                        ...
                        "<td>" . $data_select_anggota["kode_anggota"] . "</td>";
                        echo

                        "<td>" . $data_select_anggota["nama_anggota"] . "</td>";
                        if (isset($_SESSION["NamaPengguna"])) {
                            echo "<td><a href='?p=edit-anggota&id=" . $data_select_anggota["id"] . "' class='btn btn-warning'>Edit</a></td>";
                            echo "<td>
                    <form action='?p=anggota' method='post' onsubmit='return confirm(`Apakah
                        Anda yakin menghapus data ini?`)'><input type='hidden'
                        name='hdnID' value='" . $data_select_anggota["id"] . "'><button
                        type='submit' class='btn btn-danger' name='cmdHapus'
                        value='Hapus'>Hapus</button></form></td>";
                        }
                        echo "</tr>";
                        $no++;
                    }
